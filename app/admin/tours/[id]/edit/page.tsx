...
import { TourService, UpdateTourRequest } from "@/services/tour-service"
...
// Validation schema (aligned with New page)
const tourSchema = z.object({
  ...
  meeting_point: z
    ...
    .max(500, "El punto de encuentro no puede exceder 500 caracteres"),
...
export default function EditTourPage() {
  ...
  const tourId = useMemo(() => (Array.isArray(params?.id) ? params.id[0] : params?.id), [params])
  ...
  // Load tour (admin view: do NOT filter by is_active)
  useEffect(() => {
    const loadTour = async () => {
      ...
      try {
        ...
        form.reset({
          ...
          full_description: (data as any).full_description || "",
          city: data.city || "",
          location: (data as any).location || "",
          meeting_point: (data as any).meeting_point || "",
          price: Number(data.price) || 0,
          original_price: (data as any).original_price ?? undefined,
          duration: String(data.duration || ""),
          max_group_size: Number((data as any).max_group_size || 10),
          highlights: (data as any).highlights?.length ? (data as any).highlights : [""],
          included: (data as any).included?.length ? (data as any).included : [""],
          cancellation_policy: (data as any).cancellation_policy || "",
          images: (data as any).images || [],
          is_featured: Boolean(data.is_featured),
          is_active: Boolean(data.is_active),
        })
        setImageUrls(((data as any).images as string[]) || [])
      } catch (err: any) {
        console.error("Error loading tour:", err)
        toast.error(err?.message || "Error al cargar el tour")
      } finally {
        setLoading(false)
      }
    }
  ...
  const updateTourMutation = useMutation({
    mutationFn: async (data: UpdateTourRequest) => TourService.updateTour(tourId as string, data),
    onSuccess: () => {
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-muted-foreground">Actualiza la información del tour</p>
        </div>
      </div>
        ...
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
          <div className="grid gap-6 lg:grid-cols-3">
            ...
            <div className="lg:col-span-2 space-y-6">
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        <FormDescription>Ubicación general del tour</FormDescription>
                        <FormMessage />
                      </FormItem>
                    )}
                  />
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        <FormDescription>Punto específico donde inicia el tour</FormDescription>
                        <FormMessage />
                      </FormItem>
                    )}
                  />
              ...
              <Card>
                <CardHeader>
                  <CardTitle>Precios y Logística</CardTitle>
                </CardHeader>
                <CardContent className="space-y-4">
              ...
              <Card>
                ...
                <CardContent className="space-y-4">
                  ...
                  <Button type="button" variant="outline" onClick={addHighlight} className="w-full">
                    <Plus className="h-4 w-4 mr-2" /> Agregar Destacado
                  </Button>
                </CardContent>
              </Card>
              ...
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>Servicios incluidos en el tour</CardDescription>
                </CardHeader>
                <CardContent className="space-y-4">
                  ...
                  <Button type="button" variant="outline" onClick={addIncluded} className="w-full">
                    <Plus className="h-4 w-4 mr-2" /> Agregar Servicio Incluido
                  </Button>
                </CardContent>
              </Card>
            </div>
                    ...
                    render={({ field }) => (
                      <FormItem className="flex flex-row items-center justify-between rounded-lg border p-4">
                        <div className="space-y-0.5">
                          ...
                          <FormDescription>El tour será visible para los usuarios</FormDescription>
                        </div>
                        <FormControl>
                    ...
                    render={({ field }) => (
                      <FormItem className="flex flex-row items-center justify-between rounded-lg border p-4">
                        <div className="space-y-0.5">
                          ...
                          <FormDescription>Aparecerá en la sección de tours destacados</FormDescription>
                        </div>
                        <FormControl>
                          <Checkbox checked={field.value} onCheckedChange={field.onChange} />
                        </FormControl>
              ...
              <Card>
                <CardHeader>
                  <CardTitle className="flex items-center gap-2">
                    <Upload className="h-5 w-5" /> Imágenes del Tour
                  </CardTitle>
                  <CardDescription>
                    Sube/gestiona las imágenes del tour. La primera imagen será la principal.
                  </CardDescription>
                </CardHeader>
                <CardContent>
                  <ImageUpload
              ...
              <Card>
                <CardContent className="pt-6">
                  <div className="space-y-2">
                    <Button type="submit" className="w-full" disabled={updateTourMutation.isPending}>
