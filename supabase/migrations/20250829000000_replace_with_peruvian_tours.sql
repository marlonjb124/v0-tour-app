-- =============================================
-- MIGRATION: Replace Spanish tours with Peruvian tours
-- Date: 2025-08-29
-- Purpose: Update existing tour data to reflect authentic Peruvian destinations
-- =============================================

-- First, delete existing sample tours and their availability
DELETE FROM public.tour_availability WHERE tour_id IN (SELECT id FROM public.tours);
DELETE FROM public.bookings WHERE tour_id IN (SELECT id FROM public.tours);
...
-- Insert availability for all tours (next 60 days)
...
SELECT 
  ...
  CASE 
    WHEN t.duration LIKE '%completo%' OR t.duration LIKE '%día%' THEN '["08:00"]'::jsonb
    WHEN t.duration LIKE '%noche%' THEN '["08:00"]'::jsonb
